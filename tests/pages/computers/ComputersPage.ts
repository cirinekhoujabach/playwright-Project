...
export class ComputersPage extends BaseAction {
  ...
  public async addFirstComputerToCart(productName: string) {
    await this.clickElements(COMPUTER_LOCATORS.computerItem(productName));
    await this.clickElements(COMPUTER_LOCATORS.firstComputerAddToCart);
    await this.page.waitForTimeout(1000); // Attendre que le produit soit ajouté au panier
  }
  ...
  public async goToShoppingCart() {
   ...
   public async verifyProductDisplayed(productName: string) {
  await this.assertions.expectVisible(COMPUTER_LOCATORS.cartItemByName(productName));
}
